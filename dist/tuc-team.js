// Script start for click on button view more button
...
// Function to handle pagination for a specific wrapper
async function handlePagination(wrapper) {
  const paginationNext = wrapper.querySelector(".w-pagination-next");

  if (!paginationNext) return;

  while (isVisible(paginationNext)) {
    console.log("Clicking pagination button...");
    paginationNext.click();

    await new Promise((resolve) => setTimeout(resolve, 3000));
  }

  console.log("Pagination complete - button is now hidden");
}

async function handleAllPaginations() {
  if (!isSmallScreen()) {
    console.log(
      "Screen width is 767px or larger - pagination handling skipped"
    );
    return;
  }
  ...
  const wrappers = document.querySelectorAll('div[team="wrapper"]');

  console.log(
    `Small screen detected (below 767px)! Found ${wrappers.length} wrappers to check`
  );

  for (const wrapper of wrappers) {
    await handlePagination(wrapper);
  }

  console.log("All pagination checks complete");
}
handleAllPaginations().catch((err) => {
  console.error("Error during pagination handling:", err);
});

window.addEventListener("resize", () => {
  if (isSmallScreen()) {
    handleAllPaginations().catch((err) => {
      console.error("Error during pagination handling after resize:", err);
    });
  }
});
